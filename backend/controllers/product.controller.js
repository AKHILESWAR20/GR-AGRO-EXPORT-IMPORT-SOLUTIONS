...
// SECTION: Products with Cloudinary Images
// ─────────────────────────────────────────
...
const addProduct = async (req, res) => {
  try {
    ...
    // Cloudinary returns the secure_url directly in req.file.path
    const imageUrl = req.file ? req.file.path : null;

    await query(
      `INSERT INTO products (name, category, description, price, unit, origin_country, stock_status, image_url)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
      [name, category, description||null, parseFloat(price),
       unit||null, originCountry||null, stockStatus||"Available", imageUrl]
    );
  ...
  try {
    ...
    if (req.file) {
      const existing = await query(
        `SELECT image_url FROM products WHERE product_id = $1`, [id]
      );
      if (existing.rows[0]?.image_url) {
        try {
          // Extract public_id from URL
          const parts   = existing.rows[0].image_url.split("/");
...
// ── DELETE PRODUCT (also deletes Cloudinary image)
  ...
  try {
    ...
    if (existing.rows[0]?.image_url) {
      try {
        ...
        const folder = parts[parts.length - 2];
        const file   = parts[parts.length - 1].split(".")[0];
        await cloudinary.uploader.destroy(`gr-agro/products/${file}`);
      } catch(e) { /* ignore */ }
    }
